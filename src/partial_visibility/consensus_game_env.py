...
class ConsensusGameEnv:
    def __init__(self, config: Config, training: bool):
        ...
        assert self.num_slots % self.n_players == 0, "Number of slots must be divisible by number of players."
    ...
    def reset(self, slots: torch.Tensor, attn: torch.Tensor, labels: torch.Tensor, random_starting_player: bool = True):
        ...
        self.partial_slots = slots.view(slots.shape[0], self.n_players, -1, slots.shape[-1])
    ...
    def get_state(self):
        ...
        return [
            ...
            self.partial_slots[:, self.curr_player],    # shape: [B, num_slots // num_players, slot_size]
    ...
    def do_action(self, player: int, actions: tuple):
        ...
        # Readjust slot_indices
        slot_indices = slot_indices.clone().detach() + player * self.num_slots // self.n_players
        
        self.slot_selection[:, player, self.num_stages] = slot_indices
