...
from src.runtime import reconstruct_autoencoder, seed_all
from src.selector_v1_ac import ACConfig, SlotSelectorAC, evaluate_greedy, multiclass_metrics, rollout_actor_critic
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use a learned query cross-attending to all slots for step-0 slot selection only.",
    )
...
def train_one_epoch(
    ...
    *,
    ...
    if logits_all:
        metrics.update(multiclass_metrics(torch.cat(logits_all, dim=0), torch.cat(labels_all, dim=0), model.cfg.num_classes))
    else:
...
def main() -> None:
    ...
    ac_cfg = ACConfig(
        ...
        premature_stop_coef=args.premature_stop_coef,
        future_gain_margin=args.future_gain_margin,
